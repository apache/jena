...
public class VectorQuery {
    ...
    public static final Symbol vectorIndex = Symbol.create(NS + "index");
    ...
    public static void init() {
        ...
        synchronized(lock) {
            if (initialized)
                return;
            initialized = true;
            JenaSystem.logLifecycle("VectorQuery.init - start");
            VectorAssembler.init();
            PropertyFunctionRegistry.get().put(NS + "query", new PropertyFunctionFactory() {
                @Override
                public PropertyFunction create(String uri) {
                    return new VectorQueryPF();
                }
            });
            JenaSystem.logLifecycle("VectorQuery.init - finish");
        }
    }
}
