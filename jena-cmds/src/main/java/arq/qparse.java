...
public class qparse extends CmdARQ {
    ...
    protected void processModulesAndArgs() {
        ...
        for ( String arg : getValues(argDeclPrint) ) {
            switch (lowercase(arg)) {
                case "query" ->                printQuery = true;
                case "op", "alg", "algebra" -> printOp = true;
                case "quad", "quads" ->        printQuad = true;
                case "plan" ->                 printPlan = true;
                case "opt" ->                  printOpt = true;
                case "optquad", "quadopt" ->   printQuadOpt = true;
                case "none" ->                 printNone = true;
                default -> throw new CmdException("Not a recognized print form: " + arg
                                                + " : Choices are: query, op, quad, opt, optquad, plan");
            }
        }
    ...
    protected void exec() {
        try {
            ...
            if ( printPlan ) {
                divider();
                // This forces internal query initialization - must be after
                // QueryUtils.checkQuery
