...
public class TupleTable implements Sync, Closeable {
    ...
    private final TupleIndex scanAllIndex;   // Use this index if a complete scan is
                                             // needed.
    ...
    public TupleTable(int tupleLen, TupleIndex[] indexes) {
        ...
        if ( indexes[0] == null )
            throw new TDB1Exception("TupleTable: no primary index");
        for ( TupleIndex index : indexes ) {
            if ( index != null && index.getTupleLength() != tupleLen )
                throw new TDB1Exception("Incompatible index: " + index.getMappingStr());
        }
    ...
    public boolean add(Tuple<NodeId> t) {
        if ( tupleLen != t.len() )
            throw new TDB1Exception(format("Mismatch: inserting tuple of length %d into a table of tuples of length %d", t.len(), tupleLen));
        ...
        for ( int i = 0 ; i < indexes.length ; i++ ) {
            ...
            if ( !indexes[i].add(t) ) {
                ...
                throw new TDB1Exception(format("Secondary index duplicate: %s -> %s", indexes[i].getMappingStr(), t));
            }
            syncNeeded = true;
        }
        return true;
...
//        } catch (IOException ex) {}
    ...
    public boolean delete(Tuple<NodeId> t) {
        if ( tupleLen != t.len() )
            throw new TDB1Exception(format("Mismatch: deleting tuple of length %d from a table of tuples of length %d", t.len(), tupleLen));
...
//        }
        ...
        if ( tupleLen != pattern.len() )
            throw new TDB1Exception(format("Mismatch: finding tuple of length %d in a table of tuples of length %d", pattern.len(),
                                          tupleLen));
...
//    }
    ...
    /**
     * Get all indexes - for code that manipulates internal structures directly - use
     * with care
     */
    ...
    /**
     * Set index - for code that manipulates internal structures directly - use with
     * care
     */
