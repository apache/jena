    ...
    private void testForConsistency()
    {
        ...
        for ( ; iter1.hasNext() ; )
        {
            ...
            if ( !id2node_Cache.containsKey(nId) )
                throw new TDB1Exception("Inconsistent: "+n+" => "+nId) ;
            if ( notPresent.contains(n) )
                throw new TDB1Exception("Inconsistent: "+n+" in notPresent cache (1)") ;
        }
        ...
        for ( ; iter2.hasNext() ; )
        {
            ...
            if ( !node2id_Cache.containsKey(n) )
                throw new TDB1Exception("Inconsistent: "+nId+" => "+n) ;
            if ( notPresent.contains(n) )
                throw new TDB1Exception("Inconsistent: "+n+" in notPresent cache (2)") ;
        }
