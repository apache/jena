...
public class SystemTDB
{
    ...
    public static final Logger syslog = LoggerFactory.getLogger(TDB1.class) ;
    /** Send warnings and error */
    public static final Logger errlog = LoggerFactory.getLogger(TDB1.class) ;
...
//    { return SystemTDB.NullOut ; }
    ...
    public static void panic(Class<?> clazz, String string)
    {
        ...
        throw new TDB1Exception(string) ;
    }

    public static Symbol allocSymbol(String shortName)
    {
        if ( shortName.startsWith(SystemTDB.tdbSymbolPrefix))
            throw new TDB1Exception("Symbol short name begins with the TDB namespace prefix: "+shortName) ;
        if ( shortName.startsWith("http:"))
            throw new TDB1Exception("Symbol short name begins with http: "+shortName) ;
    ...
    private static Properties readPropertiesFile()
    {
        ...
        try
        {
            TDB1.logInfo.info("Using properties from '"+propertyFileName+"'") ;
    ...
    private static boolean determineIf64Bit()
    {
        ...
        if ( s != null )
        {
            ...
            TDB1.logInfo.debug("System architecture: "+(b?"64 bit":"32 bit")) ;
            return b ;
        }
        // Not a SUN VM
        ...
        TDB1.logInfo.debug("System architecture: (from java.vm.info) "+(b?"64 bit":"32 bit")) ;
        return b ;
    }
    ...
    private static FileMode determineFileMode()
    {
        ...
        {
            TDB1.logInfo.info("File mode: direct (forced)") ;
            return FileMode.direct ;
        }
        if ( x.equalsIgnoreCase("mapped") )
        {
            TDB1.logInfo.info("File mode: mapped (forced)") ;
        ...
        if ( x.equalsIgnoreCase("default") )
        {
            if ( is64bitSystem )
            {
                TDB1.logInfo.debug("File mode: Mapped") ;
                return FileMode.mapped ;
            }
            TDB1.logInfo.debug("File mode: Direct") ;
            return FileMode.direct ;
        }
        throw new TDB1Exception("Unrecognized file mode (not one of 'default', 'direct' or 'mapped': "+x) ;
    }
}
