...
package org.apache.jena.tdb1.store.nodetupletable ;
...
public class NodeTupleTableConcrete implements NodeTupleTable
{
    ...
    {
        if (indexes.length == 0 || indexes[0] == null) throw new TDB1Exception("A primary index is required") ;
        for (TupleIndex index : indexes)
        {
            if (N != index.getTupleLength())
                throw new TDB1Exception(format("Inconsistent: TupleTable width is %d but index %s is %d",
