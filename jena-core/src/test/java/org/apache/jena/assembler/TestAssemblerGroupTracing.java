...
public class TestAssemblerGroupTracing extends AssemblerTestBase {
    ...
    public void testFail() {
        ...
        g.implementWith(ModelTestLib.resource("A"), new ShantAssemble());
        try {
            g.open(root);
            fail("shouldn't get past exception");
        } catch (AssemblerException e) {
            AssemblerGroup.Frame frame = new AssemblerGroup.Frame(ModelTestLib.resource("x"), ModelTestLib.resource("A"), ShantAssemble.class);
            assertEquals(JenaTestLib.listOfOne(frame), e.getDoing());
        }
    }
