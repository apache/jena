...
public class TestModelContent extends AssemblerTestBase {
    ...
    public void testContentTransactionsNone() {
        ...
        Resource root = resourceInModel("x rdf:type ja:Model; x ja:content y; y rdf:type ja:Content; y rdf:type ja:LiteralContent; y ja:literalContent '_:x\\srdf:value\\s17\\s.'");
        try {
    ...
    public void testContentTransactionsCommit() {
        ...
        assertEquals(JenaTestLib.listOfStrings("supports[true] begin add commit"), history);
    ...
    public void testContentTransactionsAbort() {
        ...
        try {
            Resource root = resourceInModel("x rdf:type ja:Model; x ja:content y; y rdf:type ja:Content"
                                            + "; y rdf:type ja:LiteralContent; y ja:literalContent '_:x\\srdf:value\\s17\\s.'");
            a.open(Assembler.content, root);
            fail("should throw (wrapped) failing exception");
        } catch (Exception e) {
            assertEquals(JenaTestLib.listOfStrings("supports[true] begin add abort"), history);
            ModelTestLib.assertIsoModels(expected, toDeliver);
        }
    }

    protected void testModelLoadsSingleContent(Assembler a, Resource type) {
        Resource root = resourceInModel("x rdf:type " + type + "; x ja:content y; y rdf:type ja:Content"
                                        + "; y rdf:type ja:LiteralContent; y ja:literalContent '_:x\\srdf:value\\s17\\s.'");
        Model m = (Model)a.open(Assembler.content, root);
        ModelTestLib.assertIsoModels(model("_x rdf:value '17'xsd:integer"), m);
    }

    protected void testModelLoadsMultipleContent(Assembler a, Resource type) {
        Model m = (Model)a.open(Assembler.content,
                                resourceInModel("x rdf:type " + type + "; x ja:content y; x ja:content z"
        ...
        ModelTestLib.assertIsoModels(model("_x rdf:value '17'xsd:integer; _y rdf:value '42'xsd:integer"), m);
    }
}
