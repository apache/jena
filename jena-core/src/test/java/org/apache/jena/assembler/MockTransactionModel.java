 ...
 * transactions and aborting on adding statements; the model logs transactions and
 * adding-of-models. For testing only.
 */
final class MockTransactionModel extends ModelAssembler {
    ...
    protected MockTransactionModel(List<String> history, Model expected, boolean supportsTransactions, boolean abortsOnAdd) {
        super();
    ...
    protected Model openEmptyModel(Assembler a, Resource root, Mode irrelevant) {
        return new ModelCom(GraphMemFactory.createDefaultGraph()) {
            ...
            @Override
            public Model commit() {
                ModelTestLib.assertIsoModels(expected, this);
                history.add("commit");
                return this;
            }
