...
public class TestTriple extends JenaTestBase {
    ...
    public void testTripleEquals() {
        ...
        Node[] nodes = new Node[]{Node.ANY, NodeFactory.createBlankNode(id), NodeFactory.createBlankNode(), NodeFactory.createLiteral(L),
            NodeFactory.createLiteral(L2), NodeFactory.createURI(U), NodeFactory.createURI(U2), NodeFactory.createVariable(N),
        ...
        for ( int i = 0 ; i < nodes.length ; i++ ) {
            for ( int j = 0 ; j < nodes.length ; j++ ) {
                for ( int k = 0 ; k < nodes.length ; k++ ) {
                    triples[i * nodes.length * nodes.length + j * nodes.length + k] = Triple.create(nodes[i], nodes[j], nodes[k]);
                }
            }
        }
                    ...
                    for ( int i2 = 0 ; i2 < nodes.length ; i2++ ) {
                        for ( int j2 = 0 ; j2 < nodes.length ; j2++ ) {
                            for ( int k2 = 0 ; k2 < nodes.length ; k2++ ) {
                                expected[i1 * nodes.length * nodes.length + j1 * nodes.length + k1][i2 * nodes.length * nodes.length
                                                                                                    + j2 * nodes.length
                                                                                                    + k2] = nodes[i1].equals(nodes[i2])
    ...
    public void testMatchesNodes() {
        ...
        assertTrue(NodeCreateUtils.createTriple("S ?? O").matches(GraphTestLib.node("S"), GraphTestLib.node("Q"), GraphTestLib.node("O")));
        assertTrue(NodeCreateUtils.createTriple("S P ??").matches(GraphTestLib.node("S"), GraphTestLib.node("P"), GraphTestLib.node("I")));
        /* */
        assertFalse(NodeCreateUtils.createTriple("S P O").matches(GraphTestLib.node("Z"), GraphTestLib.node("P"), GraphTestLib.node("O")));
        assertFalse(NodeCreateUtils.createTriple("S P O").matches(GraphTestLib.node("S"), GraphTestLib.node("Q"), GraphTestLib.node("O")));
    ...
    public void testTripleMaps() {
        ...
        assertEquals(GraphTestLib.node("z"), getObject.apply(NodeCreateUtils.createTriple("x P z")));
    }
