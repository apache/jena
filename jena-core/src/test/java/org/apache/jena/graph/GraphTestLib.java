...
public class GraphTestLib {
    ...
    /**
     * Answer a triple described by the three space-separated node descriptions in
     * <code>fact</code>; a shorthand for
     * <code>NodeCreateUtils.createTriple(fact)</code>, which see.
     */
    ...
    /**
     * Answer a triple described by the three space-separated node descriptions in
     * <code>fact</code>, using prefix-mappings from <code>pm</code>; a shorthand for
     * <code>Triple.create(pm, fact)</code>, which see.
     */
     ...
     * semi-separated substrings of <code>facts</code>, as per <code>triple</code>
     * with prefix-mapping <code>Extended</code>.
     */
    ...
    /**
     * Answer a set of triples where the elements are described by the semi-separated
     * substrings of <code>facts</code>, as per <code>triple</code>.
     */
    ...
    /**
     * Answer a list of nodes, where the nodes are described by the space-separated
     * substrings of <code>items</code> as per <code>node()</code>.
     */
    ...
    /**
     * Answer an array of nodes, where the nodes are described by the space-separated
     * substrings of <code>items</code> as per
     */
     ...
     * <code>s</code> in the fashion of <code>tripleArray</code>, a semi-separated
     * sequence of space-separated node descriptions.
     */
     ...
     * <code>s</code> in the fashion of <code>graphAdd()</code>. Not over-ridable; do
     * not use for abstraction.
     */
    ...
    /**
     * Assert that the graph <code>g</code> is isomorphic to the graph described by
     * <code>template</code> in the fashion of <code>graphWith</code>.
     */
     ...
     * desired graph <code>expected</code>; if not, display a readable description of
     * both graphs.
     */
    ...
    /**
     * Answer a string which is a newline-separated list of triples (as produced by
     * niceTriple) in the graph <code>g</code>. The map <code>bnodes</code> maps
     * already-seen bnodes to their "nice" strings.
     */
     ...
     * triple <code>t</code> on a new line, using (and updating) <code>bnodes</code>
     * to supply "nice" strings for any blank nodes.
     */
    public static void niceTriple(StringBuilder b, Map<Node, Object> bnodes, Triple t) {
        b.append("\n    ");
     ...
     * re-use any existing string for it from <code>bnodes</code> or make a new one
     * of the form <i>_bNNNN</i> with NNNN a new integer.
     */
    public static void appendNode(StringBuilder b, Map<Node, Object> bnodes, Node n) {
        b.append(' ');
        if ( n.isBlank() ) {
     ...
     */
    public static String nice(Node n) {
        return n.toString(PrefixMapping.Extended);
    }

    /**
     * Assert that the computed graph <code>got</code> is isomorphic with the desired
     * graph <code>expected</code>; if not, fail with a default message (and pretty
     ...
     * <code>s</code>; if not, fail with pretty output of both graphs and a message
    ...
    /**
     * Assert that the graph <code>g</code> contains all the triples described by the
     * string <code>s</code; if not, fail with a message containing
     <code>name</code>.
     */
    ...
    /**
     * Assert that the graph <code>g</code> does not contain the triple described by
     * <code>s<code>; if it does, fail with a message containing
     <code>name</code>.
     */
    ...
    /**
     * Assert that the graph <code>g</code> contains none of the triples described by
     * <code>s</code> in the usual way; otherwise, fail with a message containing
     * <code>name</code>.
     */
    ...
    /**
     * Assert that <code>g</code> contains the triple described by <code>fact</code>
     * in the usual way.
     */
     ...
     * constructor is run on <code>style</code> to get the instance. Otherwise, if it
     * has a # constructor that takes an argument of <code>wrap</code>'s class before
     * the <code>ReificationStyle</code>, that constructor is used; this allows
     * non-static inner classes to be used for <code>graphClass</code>, with
     * <code>wrap</code> being the outer class instance. If no suitable constructor
     * exists, a JenaException is thrown.
     *
