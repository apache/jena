...
public class TestNode extends JenaTestBase {
    ...
    public void testNodeVariableConstructor() {
        ...
        JenaTestLib.assertDiffer(NodeFactory.createVariable("hello"), new Node_Variable("world"));
     ...
     * off, to make sure that caching effects don't hide the effect of using
     * .equals(). The strings are "equality groups": the nodes should test equal iff
     * their associated strings test equal.
     */
    @SuppressWarnings("deprecation")
    private Object[][] eqTestCases() {
        ...
        return new Object[][]{{Node.ANY, "0"}, {NodeFactory.createBlankNode(id), "1"}, {NodeFactory.createBlankNode(), "2"},
            {NodeFactory.createBlankNode(id), "1"}, {NodeFactory.createLiteral(L), "3"},

            {NodeFactory.createLiteral(L2), "4"}, {NodeFactory.createLiteral(L), "3"}, {NodeFactory.createURI(U), "5"},
            {NodeFactory.createURI(U2), "6"}, {NodeFactory.createURI(U), "5"}, {NodeFactory.createVariable(N), "7"},
            {NodeFactory.createVariable(N2), "8"}, {NodeFactory.createVariable(N), "7"},
    ...
    public void testCreateBadString() {
        ...
        } catch (NullPointerException e) {} catch (JenaException e) {}
        try {
    ...
    private void testVisitorPatternNode(String ns, NodeVisitor v) {
        Node n = GraphTestLib.node(ns);
        assertEquals(n, n.visitWith(v));
    }

    private void visitExamples(NodeVisitor nv) {
        GraphTestLib.node("sortOfURI").visitWith(nv);
        GraphTestLib.node("?variableI").visitWith(nv);
        GraphTestLib.node("_anon").visitWith(nv);
        GraphTestLib.node("11").visitWith(nv);
        GraphTestLib.node("??").visitWith(nv);
        // ---
        Node s = GraphTestLib.node("uri1");
        Node p = GraphTestLib.node("uri2");
        Node o = GraphTestLib.node("uri1");
    ...
    public void testVisitorPatternValue() {
        NodeVisitor checkValue = new NodeVisitor() {
            ...
            public Object visitBlank(Node_Blank it, String label) {
                assertTrue(it.getBlankNodeLabel() == label);
                return null;
            }
...
//        assertFalse("", Node.ANY.sameTermAs(null));
    ...
    static String[] someURIs = new String[]{"http://domainy.thing/stuff/henry", "http://whatever.com/stingy-beast/bee",
        "ftp://erewhon/12345", "potatoe:rhubarb"};
    ...
    private static void assertLangString(Node n) {
        RDFDatatype dt = n.getLiteralDatatype();
        JenaTestLib.assertDiffer("", n.getLiteralLanguage());    // "" is not legal.
