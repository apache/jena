...
/**
 * An ArrayBunch implements TripleBunch with a linear search of a short-ish array of
 * Triples. The array can grow, but it only grows by 4 elements each time (because,
 * if it gets big enough for this linear growth to be bad, it should anyways have
 * been replaced by a more efficient set-of-triples implementation).
 */
class ArrayBunch implements TripleBunch {
     ...
     * ArrayBunch's are meant for <i>small</i> sets and are replaced by some sort of
     * hash- or tree- set when they get big; currently "big" means more than 9
     * elements, so that's only one growth spurt anyway.
     */
    protected void grow() {
    ...
    public Spliterator<Triple> spliterator() {
        ...
        return new Spliterator<Triple>() {
            ...
            public Spliterator<Triple> trySplit() {
                /* the number of elements here should always be small, so splitting
                 * is not wise */
                return null;
            }
