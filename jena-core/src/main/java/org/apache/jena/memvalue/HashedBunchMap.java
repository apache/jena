 ...
 */
class HashedBunchMap extends HashCommon<Object> implements BunchMap {
    ...
    /**
     * Called by HashCommon when a key is removed: remove associated element of the
    ...
    /**
     * Called by HashCommon when a key is moved: move the associated element of the
     * <code>values</code> array.
     */
    @Override
    ...
    protected final class MovedValuesIterator extends NiceIterator<TripleBunch> {
        ...
        public TripleBunch next() {
            ...
            if ( index < movedKeys.size() )
                return get(movedKeys.get(index++));
            return noElements("");
        }

        @Override
        public void forEachRemaining(Consumer<? super TripleBunch> action) {
            while (index < movedKeys.size())
                action.accept(get(movedKeys.get(index++)));
            if ( changes > initialChanges )
    ...
    /**
     * The BasicKeyIterator iterates over the <code>keys</code> array. If a .remove
     * call moves an unprocessed key underneath the iterator's index, that key value
     * is added to the <code>movedKeys</code> list supplied to the constructor.
     */
