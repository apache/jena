...
class NodeToTriplesMapMem extends NodeToTriplesMapBase {
    NodeToTriplesMapMem(Field indexField, Field f2, Field f3) {
    ...
    public ExtendedIterator<Triple> iterateAll(Triple pattern) {
        Predicate<Triple> filter = indexField.filterOn(pattern).and(f2.filterOn(pattern)).and(f3.filterOn(pattern));
    ...
    public ExtendedIterator<Triple> iterator(Node index, Node n2, Node n3) {
        ...
        return filter.hasFilter()
            ? s.iterator(new NotifyMe(indexValue)).filterKeep(filter.getFilter()) : s.iterator(new NotifyMe(indexValue));
    }
     ...
     */
    @Override
    public ExtendedIterator<Triple> iteratorForIndexed(Object y) {
