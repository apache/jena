 ...
 */
abstract class NodeToTriplesMapBase {
    /**
    ...
    /**
     * The number of triples held in this NTM, maintained incrementally (because it's
     * a pain to compute from scratch).
     */
    ...
    protected NodeToTriplesMapBase(Field indexField, Field f2, Field f3) {
    ...
    /**
     * The values (usually nodes) which appear in the index position of the stored
     * triples; useful for eg listSubjects().
     */
    ...
    public Stream<Triple> streamAll() {
        return StreamSupport.stream(bunchMap.spliterator(), false).flatMap(bunch -> StreamSupport.stream(bunch.spliterator(), false));
    }
    ...
    public Stream<Triple> stream(Node index, Node n2, Node n3) {
        ...
        return filter.hasFilter()
            ? StreamSupport.stream(s.spliterator(), false).filter(filter.getFilter()) : StreamSupport.stream(s.spliterator(), false);
    }
