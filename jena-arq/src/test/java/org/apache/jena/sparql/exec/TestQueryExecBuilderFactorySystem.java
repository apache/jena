...
/** Tests for {@link QueryExecBuilderRegistry} and {@link UpdateExecBuilderFactory}. */
public class TestQueryExecBuilderFactorySystem {
    ...
    /**
     * Register a custom QueryExecBuilderRegistry and UpdateExecBuilderRegistry
     * with the dsg which sets the specified
    ...
    private static void setQueryEngine(DatasetGraph dsg, QueryEngineFactory qef) {
        QueryExecBuilderRegistry queryReg = new QueryExecBuilderRegistry();
        QueryExecBuilderRegistry.set(dsg.getContext(), queryReg);

        UpdateExecBuilderRegistry updateReg = new UpdateExecBuilderRegistry();
        UpdateExecBuilderRegistry.set(dsg.getContext(), updateReg);

        QueryEngineRegistry qeReg = new QueryEngineRegistry();
        qeReg.add(qef);

        queryReg.add(new QueryExecBuilderFactory() {
            ...
            @Override public QueryExecBuilder create(DatasetGraph dsg, Context cxt) {
                return QueryExecDatasetBuilderMain.create().dataset(dsg)
                    .context(cxt)
                    .set(ARQConstants.registryQueryEngines, qeReg);
            }
        });
        updateReg.add(new UpdateExecBuilderFactory() {
            @Override public boolean accept(DatasetGraph dsg, Context cxt) { return dsg instanceof DatasetGraphRef; }
            @Override public UpdateExecBuilder create(DatasetGraph dsg, Context cxt) {
                return UpdateExecDatasetBuilderMain.create().dataset(dsg)
                    .context(cxt)
                    .set(ARQConstants.registryQueryEngines, qeReg);
            }
        });
