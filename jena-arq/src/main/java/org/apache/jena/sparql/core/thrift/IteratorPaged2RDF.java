...
public class IteratorPaged2RDF<E> extends IteratorPaged2E<E> {
	...
	protected Thrift2StreamRDF thrift2StreamRDF = new Thrift2StreamRDF(prefixMap, new StreamRDFBase() {
		...
		public void triple(final Triple triple) {
			IteratorPaged2RDF.this.triple(triple);
		}

		@Override
		public void quad(final Quad quad) {
			IteratorPaged2RDF.this.quad(quad);
		}
	});
	
	public IteratorPaged2RDF(InputStreamPaged inputStreamPaged) {
...
//		TRDF.protocol(inputStreamPaged);
	...
	public boolean hasNext() {
		try {
			/*
			 * Loop until next is set to a value.
			 */
