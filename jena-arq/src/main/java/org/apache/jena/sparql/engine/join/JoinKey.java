...
{
    /** Key of no variables */
    private static final JoinKey emptyKey = new JoinKey(new Var[0]);
    ...
    public static JoinKey create(Collection<Var> keys) {
        return newBuilder().addAll(keys).build();
    }

    public static JoinKey create(Var[] keys) {
        return newBuilder().addAll(keys).build();
    }
     ...
     */
    public static JoinKey createUnsafe(Var[] keys) {
        return keys.length == 0 ? emptyKey : new JoinKey(keys);
    }
