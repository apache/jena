...
public abstract class PMapTripleTable extends PMapTupleTable<ThreeTupleMap, Triple> implements TripleTable {
    ...
    public Stream<Triple> find(final Node s, final Node p, final Node o) {
        return _find(tuple(s == null ? ANY : s, p == null ? ANY : p, o == null ? ANY : o));
    }
	 ...
	 *
	 * @param t the triple pattern for which to search
    ...
    public Stream<Triple> _find(final Triple t) {
        final Node first = t.getSubject();
        final Node second = t.getPredicate();
        final Node third = t.getObject();
				...
				if (second.isConcrete()) {
					...
					return twoTuples.get(second).map(oneTuples -> {
						if (third.isConcrete()) {
							...
							return oneTuples.contains(third) ? Stream.of(detuple(first, second, third)) : empty();
						}
						debug("Using a wildcard third slot value.");
						return oneTuples.stream().map(slot3 -> detuple(first, second, slot3));
					}).orElse(empty());
				}
				debug("Using wildcard second and third slot values.");
				return twoTuples
						.flatten((slot2, oneTuples) -> oneTuples.stream().map(slot3 -> detuple(first, slot2, slot3)));
			}).orElse(empty());
		}
		debug("Using a wildcard for all slot values.");
		return threeTuples.flatten((slot1, twoTuples) -> twoTuples
				.flatten((slot2, oneTuples) -> oneTuples.stream().map(slot3 -> detuple(slot1, slot2, slot3))));
	}

    @Override
    public void add(final Triple triple) {
        final Triple t = tuple(triple);
        _add(t.getSubject(), t.getPredicate(), t.getObject());
    }

    @Override
    public void delete(final Triple triple) {
        final Triple t = tuple(triple);
