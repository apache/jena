...
public class FN_AdjustDateToTimezone extends FunctionBase {
    public FN_AdjustDateToTimezone() {
        super();
    }
    ...
    public NodeValue exec(List<NodeValue> args) {
        if ( args.size() != 1 && args.size() != 2 )
            throw new ExprEvalException("fn:adjust-date-to-timezone: Wrong number of arguments: " + args.size() + " : [wanted 1 or 2]");

        NodeValue v1 = args.get(0);
        if ( ARQ.isStrictMode() && !v1.isDate() )
            throw new ExprEvalException("fn:adjust-date-to-timezone: Arg 1 not an xsd:date : " + v1);
