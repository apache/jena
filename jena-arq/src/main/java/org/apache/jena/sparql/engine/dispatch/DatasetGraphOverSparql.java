    ...
    extends DatasetGraphBase
{
    ...
    protected abstract void execUpdate(UpdateRequest update);
    ...
    public void deleteAnyInternal(Node g, Node s, Node p, Node o) {
    ...
    public long size() {
        long result;
        try (QueryExec qExec = query(graphsCountQuery)) {
            Binding b = qExec.select().next();
            Number number = (Number)b.get(vc).getLiteralValue();
