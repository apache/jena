...
public class ChainingQueryDispatcherExecTrackerSystem
    implements ChainingQueryDispatcher
{
    @Override
    public QueryExec create(Query query, DatasetGraph dsg, Context context, QueryDispatcher chain) {
        QueryExec delegate = chain.create(query, dsg, context);
        QueryExec result = TaskEventBroker.track(context, delegate);
