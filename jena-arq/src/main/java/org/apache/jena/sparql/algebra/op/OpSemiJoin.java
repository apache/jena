...
public class OpSemiJoin extends Op2
{
    public static Op create(Op left, Op right)
    {
        return new OpSemiJoin(left, right);
    }

    private OpSemiJoin(Op left, Op right) { super(left, right); }
    ...
    { return new OpSemiJoin(newLeft, newRight); }

    @Override
    public boolean equalTo(Op op2, NodeIsomorphismMap labelMap) {
        if ( ! ( op2 instanceof OpSemiJoin) ) return false;
