 ...
 * </pre>
 *
 * where <code>S</code>, <code>P</code>, <code>O</code> is a URI, variable, literal
 * or one of the words <code>ANY</code> (matches anything), <code>VAR</code> (matches
 * a variable), <code>TERM</code> (matches a fixed URI, or literal),
 * <code>URI</code>, <code>BNODE</code>, <code>LITERAL</code> (matches one of these
 * types).
 */
...
public final class StatsMatcher {
    ...
    public static final double weightTypeO = 1000; // ? rdf:type <Object> -- Avoid as
    ...
    /**
     * Add patterns based solely on the predicate count and some guessing for a small
     * graph (less than a few thousand triples)
     */
    ...
    /**
     * Return the matching weight for the first triple match found, else apply
     * default value for fixed, unknown predicate, else return NOMATCH
     */
    ...
    private static boolean matchNode(Item node, Item item, Match details) {
        ...
        if ( node.isSymbol() ) {
            // TERM in the thing to be matched means something concrete will be
            // there.
            if ( node.equals(TERM) ) {
                if ( item.equals(TERM) ) {
                    details.termMatches++;
