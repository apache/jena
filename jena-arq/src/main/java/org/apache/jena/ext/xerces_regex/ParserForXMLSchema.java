 ...
 *
 * Version ParserForXMLSchema.java svn:831926 2009-11-02 15:38:53Z knoaman
 */
...
class ParserForXMLSchema extends RegexParser {
    ...
    static synchronized protected RangeToken getRange(String name, boolean positive) {
        if (ranges == null) {
            ...
            RX_Token tok = RX_Token.createRange();
            setupRange(tok, SPACES);
            ranges.put("xml:isSpace", tok);
            ranges2.put("xml:isSpace", RX_Token.complementRanges(tok));

            tok = RX_Token.createRange();
            setupRange(tok, DIGITS);
            ranges.put("xml:isDigit", tok);
            ranges2.put("xml:isDigit", RX_Token.complementRanges(tok));
            ...
            tok.mergeRanges((RX_Token)ranges.get("xml:isDigit"));
            ranges.put("xml:isWord", tok);
            ranges2.put("xml:isWord", RX_Token.complementRanges(tok));

            tok = RX_Token.createRange();
            setupRange(tok, NAMECHARS);
            ranges.put("xml:isNameChar", tok);
            ranges2.put("xml:isNameChar", RX_Token.complementRanges(tok));
            ...
            ranges2.put("xml:isInitialNameChar", RX_Token.complementRanges(tok));
        }
    ...
    static void setupRange(RX_Token range, String src) {
        int len = src.length();
