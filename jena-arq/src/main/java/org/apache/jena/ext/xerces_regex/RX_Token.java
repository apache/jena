 ...
 *
 * Version Token.java svn:831926 2009-11-02 15:38:53Z knoaman
 */
@SuppressWarnings({"unchecked", "rawtypes"})
class RX_Token implements java.io.Serializable {
    ...
    static {
        RX_Token.token_empty = new RX_Token(RX_Token.EMPTY);

        RX_Token.token_linebeginning = RX_Token.createAnchor('^');
        RX_Token.token_linebeginning2 = RX_Token.createAnchor('@');
        RX_Token.token_lineend = RX_Token.createAnchor('$');
        RX_Token.token_stringbeginning = RX_Token.createAnchor('A');
        RX_Token.token_stringend = RX_Token.createAnchor('z');
        RX_Token.token_stringend2 = RX_Token.createAnchor('Z');
        RX_Token.token_wordedge = RX_Token.createAnchor('b');
        RX_Token.token_not_wordedge = RX_Token.createAnchor('B');
        RX_Token.token_wordbeginning = RX_Token.createAnchor('<');
        RX_Token.token_wordend = RX_Token.createAnchor('>');

        RX_Token.token_dot = new RX_Token(RX_Token.DOT);

        RX_Token.token_0to9 = RX_Token.createRange();
        RX_Token.token_0to9.addRange('0', '9');
        RX_Token.token_wordchars = RX_Token.createRange();
        RX_Token.token_wordchars.addRange('0', '9');
        RX_Token.token_wordchars.addRange('A', 'Z');
        RX_Token.token_wordchars.addRange('_', '_');
        RX_Token.token_wordchars.addRange('a', 'z');
        RX_Token.token_spaces = RX_Token.createRange();
        RX_Token.token_spaces.addRange('\t', '\t');
        RX_Token.token_spaces.addRange('\n', '\n');
        RX_Token.token_spaces.addRange('\f', '\f');
        RX_Token.token_spaces.addRange('\r', '\r');
        RX_Token.token_spaces.addRange(' ', ' ');

        RX_Token.token_not_0to9 = RX_Token.complementRanges(RX_Token.token_0to9);
        RX_Token.token_not_wordchars = RX_Token.complementRanges(RX_Token.token_wordchars);
        RX_Token.token_not_spaces = RX_Token.complementRanges(RX_Token.token_spaces);
    }

    static RX_Token.ParenToken createLook(int type, RX_Token child) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ParenToken(type, child, 0);
    }
    static RX_Token.ParenToken createParen(RX_Token child, int pnumber) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ParenToken(RX_Token.PAREN, child, pnumber);
    }
    static RX_Token.ClosureToken createClosure(RX_Token tok) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ClosureToken(RX_Token.CLOSURE, tok);
    }
    static RX_Token.ClosureToken createNGClosure(RX_Token tok) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ClosureToken(RX_Token.NONGREEDYCLOSURE, tok);
    }
    static RX_Token.ConcatToken createConcat(RX_Token tok1, RX_Token tok2) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ConcatToken(tok1, tok2);
    }
    static RX_Token.UnionToken createConcat() {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.UnionToken(RX_Token.CONCAT); // *** It is not a bug.
    }
    static RX_Token.UnionToken createUnion() {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.UnionToken(RX_Token.UNION);
    }
    static RX_Token createEmpty() {
        return RX_Token.token_empty;
    }
    static RangeToken createRange() {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RangeToken(RX_Token.RANGE);
    }
    static RangeToken createNRange() {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RangeToken(RX_Token.NRANGE);
    }
    static RX_Token.CharToken createChar(int ch) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.CharToken(RX_Token.CHAR, ch);
    }
    static private RX_Token.CharToken createAnchor(int ch) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.CharToken(RX_Token.ANCHOR, ch);
    }
    static RX_Token.StringToken createBackReference(int refno) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.StringToken(RX_Token.BACKREFERENCE, null, refno);
    }
    static RX_Token.StringToken createString(String str) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.StringToken(RX_Token.STRING, str, 0);
    }
    static RX_Token.ModifierToken createModifierGroup(RX_Token child, int add, int mask) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ModifierToken(child, add, mask);
    }
    static RX_Token.ConditionToken createCondition(int refno, RX_Token condition,
                                                RX_Token yespat, RX_Token nopat) {
        if (COUNTTOKENS)  RX_Token.tokens ++;
        return new RX_Token.ConditionToken(refno, condition, yespat, nopat);
    }
    ...
    static protected void registerNonXS(String name) {
        if (RX_Token.nonxs == null)
            RX_Token.nonxs = new Hashtable();
        RX_Token.nonxs.put(name, name);
    }
    static protected boolean isRegisterNonXS(String name) {
        if (RX_Token.nonxs == null)
            return false;
        return RX_Token.nonxs.containsKey(name);
    }

    private static void setAlias(String newName, String name, boolean positive) {
        RX_Token t1 = (RX_Token)RX_Token.categories.get(name);
        RX_Token t2 = (RX_Token)RX_Token.categories2.get(name);
        if (positive) {
            RX_Token.categories.put(newName, t1);
            RX_Token.categories2.put(newName, t2);
        } else {
            RX_Token.categories2.put(newName, t1);
            RX_Token.categories.put(newName, t2);
        }
    }
    ...
    static private RX_Token token_grapheme = null;
    static synchronized RX_Token getGraphemePattern() {
        if (RX_Token.token_grapheme != null)
            return RX_Token.token_grapheme;

        RX_Token base_char = RX_Token.createRange();  // [{ASSIGNED}]-[{M},{C}]
        base_char.mergeRanges(RX_Token.getRange("ASSIGNED", true));
        base_char.subtractRanges(RX_Token.getRange("M", true));
        base_char.subtractRanges(RX_Token.getRange("C", true));

        RX_Token virama = RX_Token.createRange();
        for (int i = 0;  i < RX_Token.viramaString.length(); i++) {
            virama.addRange(i, i);
        }

        RX_Token combiner_wo_virama = RX_Token.createRange();
        combiner_wo_virama.mergeRanges(RX_Token.getRange("M", true));
        combiner_wo_virama.addRange(0x1160, 0x11ff); // hangul_medial and hangul_final
        combiner_wo_virama.addRange(0xff9e, 0xff9f); // extras

        RX_Token left = RX_Token.createUnion();       // base_char?
        left.addChild(base_char);
        left.addChild(RX_Token.token_empty);

        RX_Token foo = RX_Token.createUnion();
        foo.addChild(RX_Token.createConcat(virama, RX_Token.getRange("L", true)));
     ...
     */
    static private RX_Token token_ccs = null;
    static synchronized RX_Token getCombiningCharacterSequence() {
        if (RX_Token.token_ccs != null)
            return RX_Token.token_ccs;

        RX_Token foo = RX_Token.createClosure(RX_Token.getRange("M", true)); // \pM*
        foo = RX_Token.createConcat(RX_Token.getRange("M", false), foo); // \PM + \pM*
        RX_Token.token_ccs = foo;
        return RX_Token.token_ccs;
    }
     ...
     * This class represents a node in parse tree.
     */
    static class StringToken extends RX_Token implements java.io.Serializable {
     ...
     * This class represents a node in parse tree.
     */
    static class ConcatToken extends RX_Token implements java.io.Serializable {
     ...
     * This class represents a node in parse tree.
     */
    static class CharToken extends RX_Token implements java.io.Serializable {
        ...
        public String toString(int options) {
            ...
            switch (this.type) {
              ...
              case ANCHOR:
                if (this == RX_Token.token_linebeginning || this == RX_Token.token_lineend)
                    ret = ""+(char)this.chardata;
                else
                    ret = "\\"+(char)this.chardata;
                break;
     ...
     * This class represents a node in parse tree.
     */
    static class ClosureToken extends RX_Token implements java.io.Serializable {
     ...
     * This class represents a node in parse tree.
     */
    static class ParenToken extends RX_Token implements java.io.Serializable {
     ...
     */
    static class ConditionToken extends RX_Token implements java.io.Serializable {
        ...
        public String toString(int options) {
            ...
            if (refNumber > 0) {
                ret = "(?("+refNumber+")";
            } else if (this.condition.type == RX_Token.ANCHOR) {
                ret = "(?("+this.condition+")";
            } else {
                ret = "(?"+this.condition;
            }
     ...
     * (ims-ims: .... )
     */
    static class ModifierToken extends RX_Token implements java.io.Serializable {
     ...
     * for UNION or CONCAT.
     */
    static class UnionToken extends RX_Token implements java.io.Serializable {
