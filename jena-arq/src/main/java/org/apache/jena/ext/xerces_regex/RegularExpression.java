 ...
 *
 * Version RegularExpression.java svn:961928 2010-07-08 20:43:46Z knoaman
 */
...
public class RegularExpression implements java.io.Serializable {
     ...
     */
    private synchronized void compile(RX_Token tok) {
     ...
     */
    private RX_Op compile(RX_Token tok, RX_Op next, boolean reverse) {
        RX_Op ret;
        switch (tok.type) {
        case RX_Token.DOT:
            ret = RX_Op.createDot();
        ...
        case RX_Token.CHAR:
            ret = RX_Op.createChar(tok.getChar());
            ret.next = next;
            break;

        case RX_Token.ANCHOR:
            ret = RX_Op.createAnchor(tok.getChar());
            ret.next = next;
            break;

        case RX_Token.RANGE:
        case RX_Token.NRANGE:
            ret = RX_Op.createRange(tok);
            ret.next = next;
            break;

        case RX_Token.CONCAT:
            ret = next;
            if (!reverse) {
        ...
        case RX_Token.UNION:
            RX_Op.UnionOp uni = RX_Op.createUnion(tok.size());
            for (int i = 0;  i < tok.size();  i ++) {
        ...
        case RX_Token.CLOSURE:
        case RX_Token.NONGREEDYCLOSURE:
            RX_Token child = tok.getChild(0);
            int min = tok.getMin();
            int max = tok.getMax();
            if (min >= 0 && min == max) { // {n}
                ret = next;
            ...
            if (max > 0) {
                ...
                ret = next;
                for (int i = 0;  i < max;  i ++) {
                    RX_Op.ChildOp q = RX_Op.createQuestion(tok.type == RX_Token.NONGREEDYCLOSURE);
                    q.next = next;
                ...
                RX_Op.ChildOp op;
                if (tok.type == RX_Token.NONGREEDYCLOSURE) {
                    op = RX_Op.createNonGreedyClosure();
                } else {                        // Token.CLOSURE
                    op = RX_Op.createClosure(this.numberOfClosures++);
                }
                op.next = next;
        ...
        case RX_Token.STRING:
            ret = RX_Op.createString(tok.getString());
            ret.next = next;
            break;

        case RX_Token.BACKREFERENCE:
            ret = RX_Op.createBackReference(tok.getReferenceNumber());
            ret.next = next;
            break;
        ...
        case RX_Token.PAREN:
            if (tok.getParenNumber() == 0) {
                ...
                next = RX_Op.createCapture(tok.getParenNumber(), next);
                next = compile(tok.getChild(0), next, reverse);
                ret = RX_Op.createCapture(-tok.getParenNumber(), next);
            } else {
                next = RX_Op.createCapture(-tok.getParenNumber(), next);
                next = compile(tok.getChild(0), next, reverse);
                ret = RX_Op.createCapture(tok.getParenNumber(), next);
        ...
        case RX_Token.LOOKAHEAD:
            ret = RX_Op.createLook(RX_Op.LOOKAHEAD, next, compile(tok.getChild(0), null, false));
            break;
        case RX_Token.NEGATIVELOOKAHEAD:
            ret = RX_Op.createLook(RX_Op.NEGATIVELOOKAHEAD, next, compile(tok.getChild(0), null, false));
            break;
        case RX_Token.LOOKBEHIND:
            ret = RX_Op.createLook(RX_Op.LOOKBEHIND, next, compile(tok.getChild(0), null, true));
            break;
        case RX_Token.NEGATIVELOOKBEHIND:
            ret = RX_Op.createLook(RX_Op.NEGATIVELOOKBEHIND, next, compile(tok.getChild(0), null, true));
            break;

        case RX_Token.INDEPENDENT:
            ret = RX_Op.createIndependent(next, compile(tok.getChild(0), null, reverse));
            break;

        case RX_Token.MODIFIERGROUP:
            ret = RX_Op.createModifier(next, compile(tok.getChild(0), null, reverse),
                                    ((RX_Token.ModifierToken)tok).getOptions(),
                                    ((RX_Token.ModifierToken)tok).getOptionsMask());
            break;

        case RX_Token.CONDITION:
            RX_Token.ConditionToken ctok = (RX_Token.ConditionToken)tok;
            int ref = ctok.refNumber;
            RX_Op condition = ctok.condition == null ? null : compile(ctok.condition, null, reverse);
            RX_Op yes = compile(ctok.yes, next, reverse);
            RX_Op no = ctok.no == null ? null : compile(ctok.no, next, reverse);
            ret = RX_Op.createCondition(next, ref, condition, yes, no);
            break;
...
//Public
    ...
    public boolean matches(char[]  target) {
        return this.matches(target, 0,  target .length , (RegexMatch)null);
    }
    ...
    public boolean matches(char[]  target, int start, int end) {
        return this.matches(target, start, end, (RegexMatch)null);
    }
     ...
     */
    public boolean matches(char[]  target, RegexMatch match) {
     ...
     * @return Offset of the start position in <VAR>target</VAR>; or -1 if not match.
     */
    public boolean matches(char[] target, int start, int end, RegexMatch match) {
         ...
         */
        if (this.operations != null
            && this.operations.type == RX_Op.CLOSURE && this.operations.getChild().type == RX_Op.DOT) {
     ...
     * @return Offset of the start position in <VAR>target</VAR>; or -1 if not match.
     */
    public boolean matches(String  target, RegexMatch match) {
        return this.matches(target, 0,  target .length() , match);
    }
     ...
     * @return Offset of the start position in <VAR>target</VAR>; or -1 if not match.
     */
    public boolean matches(String  target, int start, int end, RegexMatch match) {
     ...
     */
    private int match(Context con, RX_Op op, int offset, int dx, int opts) {
        ...
        for (;;) {
            if (op == null || offset > con.limit || offset < con.start) {
                ...
                case RX_Op.NONGREEDYCLOSURE:
                case RX_Op.NONGREEDYQUESTION:
                    {
                        opStack.push(op);
                        dataStack.push(offset);
                        op = op.next;
                ...
                case RX_Op.LOOKAHEAD:
                case RX_Op.NEGATIVELOOKAHEAD:
                case RX_Op.LOOKBEHIND:
                case RX_Op.NEGATIVELOOKBEHIND:
                    {
                        opStack.push(op);
                        dataStack.push(dx);
                        dataStack.push(offset);
                        dx = (op.type == RX_Op.LOOKAHEAD || op.type == RX_Op.NEGATIVELOOKAHEAD) ? 1 : -1;
                        op = op.getChild();
                    }
                    break;

                case RX_Op.INDEPENDENT:
                    {
                        opStack.push(op);
                        dataStack.push(offset);
                        op = op.getChild();
                ...
                case RX_Op.CONDITION:
                    {
                        RX_Op.ConditionOp cop = (RX_Op.ConditionOp)op;
                        if (cop.refNumber > 0) {
            ...
            while (returned) {
                ...
                case RX_Op.CONDITION:
                    {
                        final RX_Op.ConditionOp cop = (RX_Op.ConditionOp)op;
                        if (0 <= retValue) {
                            op = cop.yes;
                        }
                        else if (cop.no != null) {
    ...
    boolean matchAnchor(ExpressionTarget target, RX_Op op, Context con, int offset, int opts) {
        boolean go = false;
        switch (op.getData()) {
        case '^':
    ...
    public boolean matches(CharacterIterator target) {
        return this.matches(target, (RegexMatch)null);
    }
     ...
     * @return Offset of the start position in <VAR>target</VAR>; or -1 if not match.
     */
    public boolean matches(CharacterIterator  target, RegexMatch match) {
    ...
    transient RX_Op operations = null;
     ...
     */
    void prepare() {
        if (RX_Op.COUNT)  RX_Op.nofinstances = 0;
        ...
        */
        if (RX_Op.COUNT)  System.err.println("DEBUG: The number of operations: "+RX_Op.nofinstances);
        ...
        if (!isSet(this.options, PROHIBIT_HEAD_CHARACTER_OPTIMIZATION)
            ...
            RangeToken firstChar = RX_Token.createRange();
            int fresult = this.tokentree.analyzeFirstCharacter(firstChar, this.options);
            if (fresult == RX_Token.FC_TERMINAL) {
        ...
        if (this.operations != null
            && (this.operations.type == RX_Op.STRING || this.operations.type == RX_Op.CHAR)
            && this.operations.next == null) {
            if (DEBUG)
                System.err.print(" *** Only fixed string! *** ");
            this.fixedStringOnly = true;
            if (this.operations.type == RX_Op.STRING)
            ...
            this.fixedStringTable = new BMPattern(this.fixedString, 256,
                   ...
                   && !isSet(this.options, XMLSCHEMA_MODE)) {
            RX_Token.FixedStringContainer container = new RX_Token.FixedStringContainer();
    ...
    private static final int getWordType0(char ch, int opts) {
        if (!isSet(opts, UNICODE_WORD_BOUNDARY)) {
            if (isSet(opts, USE_UNICODE_CATEGORY)) {
                return (RX_Token.getRange("IsWord", true).match(ch)) ? WT_LETTER : WT_OTHER;
            }
