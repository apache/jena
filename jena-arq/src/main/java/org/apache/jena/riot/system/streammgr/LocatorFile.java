...
/**
 * Location files in the filing system. A FileLocator can have a "current directory"
 * - this is separate from any location mapping (see {@link LocationMapper}) as it
...
public class LocatorFile implements Locator {
    ...
    /**
     * Create a LocatorFile without a specific working directory. Relative file names
     ...
     * absolute), not a URI.
     * <p>
     * For MS Window, if asked to {@link #open} a file name with a drive letter, the
    ...
    /**
     * Processing the filename for file: or relative filename and return a filename
    ...
    public TypedInputStream open(String filenameIRI) {
        ...
        try {
            ...
            log.warn("File unreadable (but exists): " + fn + " : " + ioEx.getMessage());
            return null;
        }
    }
