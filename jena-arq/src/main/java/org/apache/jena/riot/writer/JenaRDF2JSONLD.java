...
/** Convert from Jena RDFDataset (Quads/Nodes etc) to JSON-LD internal representation of a dataset */
class JenaRDF2JSONLD implements com.github.jsonldjava.core.RDFParser {
    ...
    String blankNodeOrIRIString(Node x)
    {
        if ( x.isURI() ) return x.getURI() ;
        if ( x.isBlank() ) {
            // return a valid jsond blanknode identifier for x
            String lab = x.getBlankNodeLabel();
            if (lab.startsWith("_:")) {
                // blank node's label is a valid jsonld blanknode identifier
                // we can use it
                // except if it is one that can be generated by labels
                if (!lab.startsWith(NodeToLabel.ALLOCATOR_INC_LABEL_PREFIX)) {                
                    return lab;
                }
            }
            // generate a valid, nice looking, jsonld blanknode id
            // (typically, replace long ids generated by jena (eg. 46b9fa5f:159f1461113:-7fff) by short ones eg. "_:b13")
