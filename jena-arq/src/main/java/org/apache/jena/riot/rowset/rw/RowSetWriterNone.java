...
public class RowSetWriterNone implements RowSetWriter {

    public static RowSetWriterFactory factory = lang -> {
        if ( !Objects.equals(lang, ResultSetLang.RS_None) )
            throw new ResultSetException("Results writer None asked for a " + lang);
        return new RowSetWriterNone();
    };

    private RowSetWriterNone() {}

    @Override
    public void write(OutputStream out, RowSet resultSet, Context context) {
        RowSetOps.consume(resultSet);
    }
