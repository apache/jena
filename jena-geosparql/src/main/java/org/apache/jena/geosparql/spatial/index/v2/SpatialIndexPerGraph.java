...
public class SpatialIndexPerGraph implements SpatialIndex {
    ...
    // FIXME It appear this method cannot recompute the tree for the default graph?
    /**
     ...
     */
    public static SpatialIndex recomputeIndexForGraphs(SpatialIndexPerGraph index,
                                                       DatasetGraph datasetGraph,
                                                       List<String> graphNames) throws SpatialIndexException {
        STRtreePerGraph trees = index.getIndex();
        STRtree defaultTree = trees.getDefaultTree();
        Map<Node, STRtree> namedTreeMap = trees.getNamedTreeMap();

        try (TxnCtl txn = TxnCtl.begin(datasetGraph, TxnType.READ)) {
            for (String graphName : graphNames) {
                Node g = NodeFactory.createURI(graphName);
                if (namedTreeMap.containsKey(g)) {
                ...
                STRtree oldIndexTree = trees.setNamedTree(g, indexTree);
