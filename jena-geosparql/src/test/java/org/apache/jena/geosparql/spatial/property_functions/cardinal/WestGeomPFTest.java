...
public class WestGeomPFTest {
    ...
    public void testCheckSearchEnvelope_no_wrap() {
        ...
        Set<Node> expResult = SpatialIndexTestData.asNodes(List.of(SpatialIndexTestData.LONDON_FEATURE, SpatialIndexTestData.PERTH_FEATURE));
        HashSet<Node> result = searchEnvelope.check(spatialIndex);
        assertEquals(expResult, result);
    }
    ...
    public void testCheckSearchEnvelope_wrap() {
        ...
        WestGeomPF instance = new WestGeomPF();
        SearchEnvelope searchEnvelope = instance.buildSearchEnvelope(geometryWrapper, SpatialIndexTestData.WGS_84_SRS_INFO); //Needed to initialise the search.
        Set<Node> expResult = SpatialIndexTestData.asNodes(List.of(SpatialIndexTestData.AUCKLAND_FEATURE, SpatialIndexTestData.PERTH_FEATURE, SpatialIndexTestData.HONOLULU_FEATURE));
        HashSet<Node> result = searchEnvelope.check(spatialIndex);
        assertEquals(expResult, result);
    }
