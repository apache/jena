 ...
 */
public abstract class AbstractReifierTest extends AbstractGraphProducerUser {
	...
	protected final Graph getGraphWith(String facts) {
		...
		graphAdd(getGraphProducer().newGraph(), facts);
		return result;
	}
	...
	protected final Graph graphWithUnless(boolean cond, String facts) {
		return graphWith(getGraphProducer().newGraph(), cond ? "" : facts);
	}
	...
	public void testGetGraphNotNull() {
		assertNotNull(getGraphProducer().newGraph());
	}
	...
	public void testManifestQuads() {
		...
		assertIsomorphic(graphWith(getGraphProducer().newGraph(), reified), g);
	}
