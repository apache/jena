...
public class TestTriple {
	...
	public void testTripleEquals() {
		...
		Node[] nodes = new Node[] { Node.ANY, NodeFactory.createAnon(id),
				NodeFactory.createAnon(), NodeFactory.createLiteral(L),
				NodeFactory.createLiteral(L2), NodeFactory.createURI(U),
				NodeFactory.createURI(U2), NodeFactory.createVariable(N),
		...
		for (int i = 0; i < nodes.length; i++) {
			for (int j = 0; j < nodes.length; j++) {
				for (int k = 0; k < nodes.length; k++) {
					triples[i * nodes.length * nodes.length + j * nodes.length
					...
					for (int i2 = 0; i2 < nodes.length; i2++) {
						for (int j2 = 0; j2 < nodes.length; j2++) {
							for (int k2 = 0; k2 < nodes.length; k2++) {
								expected[i1 * nodes.length * nodes.length + j1
		...
		for (int i = 0; i < triples.length; i++) {
			for (int j = 0; j < triples.length; j++) {
				if (expected[i][j]) {
					assertEquivalent("triples " + i + ", " + j, triples[i],
							triples[j]);
				} else {
					assertDiffer("triples" + i + ", " + j, triples[i],
							triples[j]);
				}
			}
		}
	...
	public void testTripleCreate() {
		Node S = NodeCreateUtils.create("s"), P = NodeCreateUtils.create("p"), O = NodeCreateUtils
				.create("o");
	...
	public void testTripleCreateFromString() {
		Node S = NodeCreateUtils.create("a"), P = NodeCreateUtils.create("_P"), O = NodeCreateUtils
				.create("?c");
	...
	public void testTriplePrefixes() {
		Node S = NodeCreateUtils.create("rdf:alpha"), P = NodeCreateUtils
				.create("dc:creator");
		Node O = NodeCreateUtils.create("spoo:notmapped");
		Triple t = NodeCreateUtils
				.createTriple("rdf:alpha dc:creator spoo:notmapped");
	...
	public void testTripleToStringOrdering() {
		...
		assertTrue(
				...
				t1.toString().indexOf("subject") < t1.toString().indexOf(
						"predicate"));
		assertTrue(
				"predicate must preceed object",
				t1.toString().indexOf("predicate") < t1.toString().indexOf(
						"object"));
	}
