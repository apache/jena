...
public class TestStoreSpeed {
	...
	public static void main(String[] args) {
		new TestStoreSpeed().gonzales("subject StoreMem",
				Factory.createGraphMem());
		new TestStoreSpeed().gonzales("normal StoreMem",
				Factory.createGraphMem());
	...
	private Triple newt(int i) {
		return new Triple(node("s" + (i % 1000)), node("p" + ((i + 11) % 20)),
				node("s" + ((i + 131) % 1001)));
	}
	...
	private void consumeAll(Graph g) {
		...
		while (it.hasNext()) {
			it.next();
			count += 1; /*
						 * if (count %1000 == 0) System.err.print( (count /
						 * 1000) %10 );
						 */
		}
