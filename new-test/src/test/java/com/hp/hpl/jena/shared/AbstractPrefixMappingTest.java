...
public abstract class AbstractPrefixMappingTest {
	...
	/**
	 * test that a PrefixMapping maps names to URIs. The names and URIs are all
	 * fully distinct - overlapping names/uris are dealt with in other tests.
	 */
	@Test
	 ...
	 * prefixes. uriB is a prefix of uriA to try and ensure that the ordering of
	...
	public void testExpandPrefix() {
		...
		for (int i = 0; i < expansions.length; i += 1)
			assertEquals("should expand correctly", expansions[i][1],
	...
	/**
	 * Test that the empty prefix does not wipe an existing prefix for the same
	 * URI.
	 */
	@Test
	...
	@Test
	public void testAllowNastyNamespace() { // we now allow namespaces to end
