...
public class HashCommonTest {
	...
	private void assertAlike(ProbeHashCommon desired, ProbeHashCommon got) {
		assertEquals("capacities must be equal", desired.capacity(),
				got.capacity());
		for (int i = 0; i < desired.capacity(); i += 1)
			assertEquals(desired.getItemForTestingAt(i),
					got.getItemForTestingAt(i));
	}
	 ...
	 * colon-separated sequence <code>index:hash:label</code>: the item
	 * <code>(hash, label)</code> will be placed at <code>index</code>. Negative
	 * index values are interpreted as indexs from the <i>end</code> of the key
	 * array, by adding the probe's capacity to them.
	 */
	protected ProbeHashCommon probeWith(String items) {
		...
		while (st.hasMoreTokens()) {
			...
			result.set((index < 0 ? index + result.capacity() : index),
					new Item(hash, w));
		}
		return result;
	}
