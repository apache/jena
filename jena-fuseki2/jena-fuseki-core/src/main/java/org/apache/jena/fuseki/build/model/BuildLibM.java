...
/*package*/ class BuildLibM {
    ...
    /*package*/ static RDFNode getOne(Resource svc, Property property) {
        ResultSet rs = BuildLibM.query("SELECT * { ?svc <" + property.getURI() + "> ?x}", svc.getModel(), "svc", svc);
        if ( !rs.hasNext() )
            throw new FusekiConfigException("No property '" + property + "' for service " + BuildLibM.nodeLabel(svc));
        RDFNode x = rs.next().get("x");
        if ( rs.hasNext() )
            throw new FusekiConfigException("Multiple properties '" + property + "' for service " + BuildLibM.nodeLabel(svc));
        return x;
    }
    ...
    /*package*/ static Collection<RDFNode> getAll(Resource resource, String property) {
        ResultSet rs = BuildLibM.query("SELECT * { ?subject " + property + " ?x}", resource.getModel(), "subject", resource);
        if ( ! rs.hasNext() )
            return null;
