...
public class TextIndexESIT extends BaseESTest {
    ...
    public void testSameLabelInDifferentLanguages() throws InterruptedException{
        ...
        List<TextHit> result =  classToTest.query(RDFS.label.asNode(), "Berlin", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());
        ...
        result =  classToTest.query(RDFS.label.asNode(), "Berlin", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());
    ...
    public void testLanguageTagSubCodes() {
        ...
        List<TextHit> result =  classToTest.query(RDFS.label.asNode(), "color", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());

        //Next Lets find it using colour
        result =  classToTest.query(RDFS.label.asNode(), "colour", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());

        //Next lets find it after specifying the lang parameter
        result =  classToTest.query(RDFS.label.asNode(), "label:colour AND lang:en*", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());

        result =  classToTest.query(RDFS.label.asNode(), "label:color AND lang:en*", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());

        //Now lets find it by specifying exact lang values
        result =  classToTest.query(RDFS.label.asNode(), "label:colour AND lang:en-GB", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());

        result =  classToTest.query(RDFS.label.asNode(), "label:color AND lang:en-US", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(1, result.size());
        Assert.assertEquals("http://example/x3", result.get(0).getNode().getURI());

        //We should NOT be able to find anything for wrong language
        result =  classToTest.query(RDFS.label.asNode(), "label:color AND lang:en-GB", 10);
        Assert.assertNotNull(result);
        Assert.assertEquals(0, result.size());
