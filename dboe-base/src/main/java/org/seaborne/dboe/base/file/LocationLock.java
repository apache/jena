...
public class LocationLock {
    ...
    private void takeLock(int pid) {
        ...
        try (BufferedWriter writer = new BufferedWriter(new FileWriter(lockFile))) {
            ...
            throw new DBOpEnvException("Failed to obtain a lock on the location " + location.getDirectoryPath(), e);
        }
    }
    ...
    public void release() {
        ...
        if (owner != ProcessUtils.getPid(NO_OWNER))
            throw new DBOpEnvException("Cannot release the lock on location " + location.getDirectoryPath()
        ...
        if (!lockFile.delete())
            throw new DBOpEnvException("Failed to release the lock on location " + location.getDirectoryPath()
