...
public class LocationLock {
	...
	public int getOwner() {
		...
		if (lockFile.exists()) {
			...
			try {
				String rawLockInfo = IO.readWholeFileAsUTF8(lockFile
						.getAbsolutePath());
				int owner = Integer.parseInt(rawLockInfo);
				return owner;
			} catch (IOException e) {
				throw new FileException(
						"Unable to check TDB lock owner due to an IO error reading the lock file",
						e);
			} catch (NumberFormatException e) {
				throw new FileException(
	...
	private void takeLock(int pid) {
		...
		try {
			...
			throw new TDBException("Failed to obtain a lock on the location "
					+ location.getDirectoryPath(), e);
		}
	}
	...
	public void release() {
		...
		if (owner != getPid())
			throw new TDBException("Cannot release the lock on location "
					+ location.getDirectoryPath()
		...
		if (!lockFile.delete())
			throw new TDBException("Failed to release the lock on location "
					+ location.getDirectoryPath()
