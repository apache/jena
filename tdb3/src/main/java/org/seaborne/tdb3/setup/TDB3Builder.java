...
package org.seaborne.tdb3.setup;
 ...
 */
public class TDB3Builder {
    private Logger log = LoggerFactory.getLogger(TDB3Builder.class) ;
    
    public static StorageTDB build(Location location) {
        return build(location, StoreParams.getDftStoreParams()) ;
    }

    public static StorageTDB build(Location location, StoreParams appParams) {
        ...
        return create(location, params).build$TDB3() ; 
    }
...
//    }
    ...
    private StorageTDB build$TDB3() {
        ...
        TransactionalSystem trans = new TransactionalBase(txnCoord) ;
        StorageTDB dsg = new StorageTDB(trans, 
                                        tripleTable, quadTable, prefixes, 
                                        ReorderLib.fixed(), location, params) ;
        QC.setFactory(dsg.getContext(), OpExecutorTDB1.OpExecFactoryTDB) ;
        txnCoord.start() ;
    ...
    private TDB3Builder(TransactionCoordinator txnCoord, Location location, StoreParams params, ComponentIdMgr componentIdMgr) {
    ...
    public static TDB3Builder create(Location location) {
        return create(location, StoreParams.getDftStoreParams()) ; 
    }
    
    public static TDB3Builder create(Location location, StoreParams params) {
        TransactionCoordinator txnCoord = buildTransactionCoordinator(location) ;
        return new TDB3Builder(txnCoord, location, params, new ComponentIdMgr(UUID.randomUUID())) ;
    }

    public static TDB3Builder create(TransactionCoordinator txnCoord, Location location, StoreParams params) {
        return new TDB3Builder(txnCoord, location, params, new ComponentIdMgr(UUID.randomUUID())) ;
    }
